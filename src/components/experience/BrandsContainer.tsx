...
const BrandsContainer = () => {
  ...
  return (
    ...
    >
      <div className="mt-20 grid w-full grid-cols-2 overflow-hidden rounded-xl border border-black">
        {brandsData.map((item, i) => (
          <div
            onMouseEnter={() => handleHover(i + 1)}
            key={item.label}
            className={`"h-[230px] relative border border-gray-500 xl:h-[150px] 2xl:h-[230px]`}
          >
            <PreloadImage
              src={item.src}
              alt="brand-image"
